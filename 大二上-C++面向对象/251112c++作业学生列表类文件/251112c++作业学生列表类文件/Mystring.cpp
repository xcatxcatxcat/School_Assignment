...
//写入文本文件
bool Mystring::write_text(std::ostream& os) const
{
	if (!os) return false;
	os << len << " "<<*this<<"\n";
	return true;
...
//读出文本文件
bool Mystring::read_text(std::istream& is)
{
	if (!is) return false;
	is >> this->len;
...
//写入二进制文件
bool Mystring::write_binary(std::ostream& os) const
{
	if (!os) return false;
	os.write((char*)&(this->len), sizeof(size_t));
	os.write(this->str, sizeof(char) * (this->len));
	return true;
}
//读出二进制文件
bool Mystring::read_binary(std::istream& is)
{
	if (!is) return false;
	size_t LEN;
	is.read((char*)&LEN, sizeof(size_t));
	char* NEW = new char[LEN + 1];
	is.read(NEW, sizeof(char) * LEN);
	NEW[LEN] = '\0';
	delete[] str;
	str = NEW;
	return true;
